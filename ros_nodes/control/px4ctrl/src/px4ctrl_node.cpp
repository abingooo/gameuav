...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber status_text_sub =
        nh.subscribe<mavros_msgs::StatusText>("/mavros/statustext/recv",
                                              50,
    ...
    ros::Subscriber hover_yaw_sub =
    nh.subscribe<std_msgs::Float64>("hover_yaw_cmd",
                                    50,
                                    &PX4CtrlFSM::hover_yaw_cmd_cb,
                                    &fsm);

    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", 10);
    fsm.local_pos_sp_pub = nh.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local", 10);
