...
namespace
{
double quaternionToYawDeg(const Eigen::Quaterniond &q)
{
  constexpr double kRadToDeg = 180.0 / 3.14159265358979323846;
  const double siny_cosp = 2.0 * (q.w() * q.z() + q.x() * q.y());
  const double cosy_cosp = 1.0 - 2.0 * (q.y() * q.y() + q.z() * q.z());
  return std::atan2(siny_cosp, cosy_cosp) * kRadToDeg;
}
} // namespace
...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/arrival_radius", arrival_radius_, 0.5);
    nh.param("fsm/arrival_distance_increase", arrival_distance_increase_, 0.2);
    nh.param("fsm/arrival_max_initial_goal_distance", arrival_max_initial_goal_distance_, 2.0);

    arrival_radius_ = std::max(0.0, arrival_radius_);
    arrival_distance_increase_ = std::max(0.0, arrival_distance_increase_);
    arrival_max_initial_goal_distance_ = std::max(0.0, arrival_max_initial_goal_distance_);
    goal_arrival_detector_.configure(arrival_radius_, arrival_distance_increase_);
    ...
    std::string swarm_trajs_out_topic;
    nh.param("manager/swarm_trajs_out_topic", swarm_trajs_out_topic, std::string("/drone_0_planning/swarm_trajs"));

    if (planner_manager_->pp_.drone_id >= 1)
    {
      std::string swarm_trajs_in_topic;
      nh.param("manager/swarm_trajs_in_topic", swarm_trajs_in_topic, std::string("/drone_0_planning/swarm_trajs"));
      swarm_trajs_sub_ = nh.subscribe(swarm_trajs_in_topic, 10, &EGOReplanFSM::swarmTrajsCallback, this, ros::TransportHints().tcpNoDelay());
    }
    swarm_trajs_pub_ = nh.advertise<traj_utils::MultiBsplines>(swarm_trajs_out_topic, 10);
    ...
    {
      planpoint_sub_ = nh.subscribe("/planning/goal", 1, &EGOReplanFSM::planpointCallback, this);
      yaw_sub_      = nh.subscribe("/planning/goal_yaw_deg", 1, &EGOReplanFSM::yawCallback, this);
      hover_yaw_cmd_pub_ = nh.advertise<std_msgs::Float64>("/px4ctrl/hover_yaw_cmd", 10);
      current_yaw_pub_ = nh.advertise<std_msgs::Float64>("/current_yaw_deg", 10);
    }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    {
      end_pt_ = next_wp;
      goal_arrival_detector_.reset();
      const double initial_goal_distance = (end_pt_ - odom_pos_).norm();
      arrival_detection_enabled_for_goal_ =
          target_type_ == TARGET_TYPE::PLAN_TARGET &&
          initial_goal_distance <= arrival_max_initial_goal_distance_;
      if (target_type_ == TARGET_TYPE::PLAN_TARGET)
      {
        ROS_INFO("Single-point pass-through detection %s: initial goal distance %.3fm, limit %.3fm.",
                 arrival_detection_enabled_for_goal_ ? "enabled" : "disabled",
                 initial_goal_distance, arrival_max_initial_goal_distance_);
      }
  ...
  void EGOReplanFSM::yawCallback(const std_msgs::Float64ConstPtr &msg)
  {
    std_msgs::Float64 out;
    out.data = msg->data; // 仍用度制，转换放到 px4ctrl 一侧统一做
    hover_yaw_cmd_pub_.publish(out);
  }
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    if (arrival_detection_enabled_for_goal_ && have_target_ && exec_state_ == EXEC_TRAJ &&
        goal_arrival_detector_.update((end_pt_ - odom_pos_).norm()))
    {
      ROS_INFO_THROTTLE(1.0,
                        "Goal pass-through detected: distance increased %.2fm from the closest point.",
                        arrival_distance_increase_);
    }

    if (target_type_ == TARGET_TYPE::PLAN_TARGET)
    {
      std_msgs::Float64 yaw_msg;
      yaw_msg.data = quaternionToYawDeg(odom_orient_);
      current_yaw_pub_.publish(yaw_msg);
    }
  ...
  void EGOReplanFSM::finishCurrentTarget(bool hold_current_position)
  {
    ...
    have_target_ = false;
    have_trigger_ = false;
    arrival_detection_enabled_for_goal_ = false;
    goal_arrival_detector_.reset();
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      const bool passed_target = goal_arrival_detector_.reached();
      const bool local_target_is_goal = (local_target_pt_ - end_pt_).norm() < 1e-3;
      const bool trajectory_finished = t_cur > info->duration_ - 1e-2;
      ...
      else if (target_type_ == TARGET_TYPE::PLAN_TARGET && passed_target)
      {
        ROS_INFO("Goal reached after confirmed pass-through; holding current position.");
        finishCurrentTarget(true);
        goto force_return;
      }
      else if (local_target_is_goal)
      {
