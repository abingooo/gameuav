...
| `cam0_calib` | `left.yaml` | 左目内参文件。 | 使用实时 `image_rect_raw` 的零畸变矫正内参；路径按 config 目录拼接。 |
...
| `body_T_cam0` | 4x4矩阵 | `body(imu) <- cam0` 的外参（旋转+平移）。 | 2026-07-21 机械结构调整后重新标定；平移约 `(0.0706, 0.0047, 0.0936) m`。 |
| `body_T_cam1` | 4x4矩阵 | `body(imu) <- cam1` 的外参（旋转+平移）。 | 同次标定后固定；平移约 `(0.0694, -0.0455, 0.0943) m`。 |

补充：生产飞行保持 `estimate_extrinsic=0`。如需重复标定，使用独立的 `fast_drone_250_online_calibration.yaml`，不要在飞行配置中开启在线外参。
...
| `F_threshold` | `1.0` | 基础矩阵 RANSAC 阈值（像素）。 | `rejectWithF()` 已启用，用于剔除错误光流匹配。 |
...
| 参数 | 当前值 | 作用 | 备注 |
|---|---:|---|---|
| `acc_n` | `0.0262231027` | 加速度计测量噪声标准差。 | 来自现有 IMU 噪声标定，用于预积分协方差。 |
| `gyr_n` | `0.0604501713` | 陀螺仪测量噪声标准差。 | 同上。 |
| `acc_w` | `0.0006844588` | 加速度计零偏随机游走标准差。 | 同上。 |
| `gyr_w` | `0.0016224084` | 陀螺仪零偏随机游走标准差。 | 同上。 |
...
| `td` | `0.001719165271` | 固定时间偏移（秒）。 | 2026-07-21 在线标定结果；按 `image_time + td` 与 IMU 对齐。 |
...
- 在线外参和时间偏移结果不会自动回写生产配置，必须验证后再固定。
