...
| `body_T_cam0` | 4x4矩阵 | `body(imu) <- cam0` 的外参（旋转+平移）。 | 2026-07-19 在线标定后固定；平移约 `(0.1973, 0.0121, -0.0586) m`。 |
| `body_T_cam1` | 4x4矩阵 | `body(imu) <- cam1` 的外参（旋转+平移）。 | 同次标定后固定；平移约 `(0.1976, -0.0372, -0.0571) m`。 |
...
| `td` | `0.012404321603` | 固定时间偏移（秒）。 | 2026-07-19 在线标定结果；按 `image_time + td` 与 IMU 对齐。 |
