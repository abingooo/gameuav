...
| `/control/spf_position` | `geometry_msgs/PoseStamped` | 输入 | control interface | SPF 直控位置目标，不经过 EGO；满足到达稳定条件后释放 |
...
| SPF position | `/control/spf_position` | SPF 专用位置目标，直接生成 `PositionCommand` 给 px4ctrl，直到新目标接管或满足到达稳定条件。 |
...
真机默认到达条件为：XY 误差不超过 `0.25 m`、Z 误差不超过 `0.20 m`、yaw
误差不超过 `10 deg`，且三维线速度不超过 `0.25 m/s`；四项条件连续满足
`0.5 s` 才算稳定。随后控制门面进入 `spf_hover_wait`，清除缓存的 SPF/EGO
运动命令并停止发布 `/control/position_cmd`。PX4Ctrl 在配置的 `0.5 s` 命令超时
后从 `CMD_CTRL` 转入 `AUTO_HOVER`。新的 SPF 目标被接受后会恢复命令发布，并使
PX4Ctrl 重新进入 `CMD_CTRL`。

单次 `/spf/user_command` 或手工 `/control/spf_position` 到达后会保持悬停；连续
`/spf/task/start` 则在轮间延时后请求下一轮 SPF。作者 SPF 不输出任务级
`final`/`done`，所以局部点到达不等于语义任务成功。上述到达释放属于
GameUAV/PX4Ctrl 接入行为，不是新增的 SPF 模型能力。连续任务进入任一终态时都会
关闭共享 `/spf/enable` 门，使活动目标和迟到的 worker 响应失效；开始下一项任务前
必须重新显式 enable。
